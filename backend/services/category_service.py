...
def update_category(user_id: int, category_id: int, name: str | None = None, kind: str | None = None) -> dict | None:
    ...
    if name is not None:
        ...
        if not name:
            raise ValueError("empty name")
        dup = (
            UserCategory.query.filter_by(user_id=user_id, name=name, kind=kind or row.kind)
