...
def format_user_context(context: dict | None) -> str:
    ...
    if reminders:
        ...
        for r in reminders[:5]:
            lines.append(f"- {r.get('title', '')}（{r.get('due_at', '')}）")
...
def answer_query(topic: str, context: dict | None) -> str:
    ...
    if topic == "expense":
        return f"今日支出合计 {expense} 元。"
    if topic == "income":
        return f"今日收入合计 {income} 元。"
    if topic == "reminders":
        if not reminders:
            return "今日暂无待办提醒。"
        parts = [r.get("title", "") for r in reminders[:5]]
        return "待办提醒：" + "；".join(parts) + "。"
    total_assets = sum(float(a.get("balance", 0)) for a in assets)
    return (
        f"今日支出 {expense} 元，收入 {income} 元；"
        f"资产账户 {len(assets)} 个，合计约 {total_assets:.2f} 元；"
